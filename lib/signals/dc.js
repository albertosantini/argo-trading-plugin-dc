...
function aroonUp(uptrends, period) {
    ...
    res = aroon(1, pexts, poss, period);

    return {
        aroonUp: res.upTrendAroonUps,
        aroonDown: res.upTrendAroonDowns
    };
}
...
function aroonDown(downtrends, period) {
    ...
    return {
        aroonUp: res.downTrendAroonUps,
        aroonDown: res.downTrendAroonDowns
    };
}
...
function aroon(trend, pexts, poss, period) {
    ...
    poss.forEach(function (pos, index) {
        ...
        if (trend === 1) {
            ...
            upTrendAroonUps.push((period - m1) / period);
            upTrendAroonDowns.push((period - m2) / period);
        }
        if (trend === -1) {
            m1 = subPoss.indexOf(Math.max.apply(null, subPoss));
            m2 = subPexts.indexOf(Math.max.apply(null, subPexts));
            downTrendAroonUps.push((period - m2) / period);
            downTrendAroonDowns.push((period - m1) / period);
        }
    });

    return {
        upTrendAroonUps: upTrendAroonUps,
        upTrendAroonDowns: upTrendAroonDowns,
        downTrendAroonUps: downTrendAroonUps,
        downTrendAroonDowns: downTrendAroonDowns
    };
}
