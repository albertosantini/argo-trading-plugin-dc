...
class LocalLiquidity {
    ...
    computation(price) {
        ...
        if (!event) {
            ...
            this.liq = 1.0 - Tools.cumNorm(Math.sqrt(this.alpha) * (this.surp - this.H1) / Math.sqrt(this.H2));
        }
     ...
     * @param {Object} price is just a new price
     * @returns {number} 1 and -1 if DC IE, 2 or -2 if OS IE, 0 otherwise.
     */
    run(price) {
        if (!price) {
            return 0;
