...
class CoastlineTrader {
    ...
    fillOrder(order) { // eslint-disable-line class-methods-use-this
        tools.log(order);

        const instrument = order.priceOpened.instrument;
        const units = 100 * order.getVolume();
        const isBuy = order.type === 1;
    ...
    makeBuyFilled(price) {
        ...
        if (this.longShort === 1) {
            ...
            this.fillOrder(this.buyLimitOrder);
    ...
    makeSellFilled(price) {
        ...
        if (this.longShort === -1) {
            ...
            this.fillOrder(this.sellLimitOrder);
        } else {
     ...
     */
    computeLiqUniteCoef(liquidity) { // eslint-disable-line class-methods-use-this
        let liqUnitCoef;
    ...
    computeTargetRelatPnL(limitOrder) { // eslint-disable-line no-unused-vars, class-methods-use-this
        // TODO
    }
